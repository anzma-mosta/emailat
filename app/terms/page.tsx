'use client';

import { useLocale } from '@/components/providers/LocaleProvider';

export default function TermsPage() {
  const { t } = useLocale();
  return (
    <div className="bg-background-light dark:bg-background-dark text-[#0c121d] dark:text-white transition-colors duration-200">
      <div className="relative flex min-h-screen w-full flex-col overflow-x-hidden font-display">
        <main className="layout-container flex grow flex-col items-center py-10 px-4 md:px-10">
          <div className="layout-content-container max-w-[880px] w-full space-y-8">
            <div className="flex flex-col gap-2">
              <h1 className="text-3xl md:text-4xl font-black tracking-tight">{t('terms.title')}</h1>
              <p className="text-gray-600 dark:text-gray-400">{t('terms.lastUpdated')}</p>
            </div>

            <section className="bg-white dark:bg-[#1a202c] rounded-xl border border-[#e6ebf4] dark:border-gray-700 p-6 shadow-sm">
              <p className="text-gray-700 dark:text-gray-300">{t('terms.intro')}</p>
            </section>

            <section className="bg-white dark:bg-[#1a202c] rounded-xl border border-[#e6ebf4] dark:border-gray-700 p-6 shadow-sm">
              <h2 className="text-xl font-bold mb-2">{t('terms.acceptanceTitle')}</h2>
              <p className="text-gray-700 dark:text-gray-300">{t('terms.acceptanceText')}</p>
            </section>

            <section className="bg-white dark:bg-[#1a202c] rounded-xl border border-[#e6ebf4] dark:border-gray-700 p-6 shadow-sm">
              <h2 className="text-xl font-bold mb-2">{t('terms.billingTitle')}</h2>
              <p className="text-gray-700 dark:text-gray-300">{t('terms.billingText')}</p>
            </section>

            <section className="bg-white dark:bg-[#1a202c] rounded-xl border border-[#e6ebf4] dark:border-gray-700 p-6 shadow-sm">
              <h2 className="text-xl font-bold mb-2">{t('terms.privacyTitle')}</h2>
              <p className="text-gray-700 dark:text-gray-300">{t('terms.privacyText')}</p>
            </section>
          </div>
        </main>
      </div>
    </div>
  );
}
